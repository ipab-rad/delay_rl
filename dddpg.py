...
class DDDPGAgent(Agent):
    """Write me
    """
    def __init__(self, nb_actions, actor, critic, critic_action_input, memory, 
                 model, nb_delay_steps=0,
                 gamma=.99, batch_size=32, nb_steps_warmup_critic=1000, nb_steps_warmup_actor=1000,
                 train_interval=1, memory_interval=1, delta_range=None, delta_clip=np.inf,
                 random_process=None, custom_model_objects={}, target_model_update=.001, **kwargs):
        if hasattr(actor.output, '__len__') and len(actor.output) > 1:
            raise ValueError('Actor "{}" has more than one output. DDDPG expects an actor that has a single output.'.format(actor))
        if hasattr(critic.output, '__len__') and len(critic.output) > 1:
            raise ValueError('Critic "{}" has more than one output. DDDPG expects a critic that has a single output.'.format(critic))
        ...
        super(DDDPGAgent, self).__init__(**kwargs)
        ...
        # Forward model
        self.model = model
        self.nb_delay_steps = nb_delay_steps
    ...
    def backward(self, reward, terminal=False):
        ...
        if can_train_either and self.step % self.train_interval == 0:
            ...
            # Update forward model, if warm up is over.
            if self.step > self.nb_steps_warmup_actor:
                #state0_batch_with_action = [state0_batch]
                #state0_batch_with_action.insert(self.critic_action_input_idx, action_batch)
                self.model.fit(x=[state0_batch, action_batch], y=state1_batch)
