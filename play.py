...
from keras.optimizers import Adam#, AdditionalUpdatesOptimizer
from dddpg import DDDPGAgent
...
ap.add_argument("-g", "--gpu", required=True,
                help="GPU number")
ap.add_argument("-a", "--actor_delay", required=True,
                help="Actor delay to load")
...
actor_delay_name = args['actor_delay']
...
print("****\n Environment, Steps Delay, GPU", ENV_NAME, delay_steps, args["gpu"], "\n**********")
# Next, we build a very simple model.
actor = load_model("logs/actor_pendulum_delay_" + actor_delay_name + ".h5")
print(actor.summary())
#action_input = Input(shape=(nb_actions,), name='action_input')
#critic = load_model("logs/critic_pendulum_delay_0.h5")#, custom_object={'optimizer': AdditionalUpdatesOptimizer})
#print(critic.summary())

# TODO can't load the critic, so I create a empty one, is not needed for
# exploitation but for creating the DDPG agent
action_input = Input(shape=(nb_actions,), name='action_input')
...
agent = DDDPGAgent(nb_actions=nb_actions, actor=actor, critic=critic, critic_action_input=action_input,
...
agent.test(delayed_env, nb_episodes=5, visualize=True, nb_max_episode_steps=200)
